...
bool GameScene::init(){
	...
	do 
	{
		...
		CC_BREAK_IF(!initPlayer());//初始化玩家
		CC_BREAK_IF(!xiPai());//洗牌
		CC_BREAK_IF(!initAnNiu());//初始化按钮
		CC_BREAK_IF(!InitNpcBuChuLable());//生成两个NPC不出的lebal
...
Poker* GameScene::selectPoker(int huaSe,int num){
	Poker* pk;
	if(huaSe != Gui)
		pk = Poker::create("poker.png",CCRect(num*pkWidth, huaSe*pkHeight, pkWidth, pkHeight));//牌的位置，牌的长宽
	else
...
bool GameScene::createPokers(){
	...
	do 
	{
		...
		Poker* pk;
		//创建52个牌
		for (int i=0; i<4; ++i)//四个花色
		{
			for (int j=0; j<13; ++j)//每个花色十三张牌
			{
				pk = selectPoker(i,j);
...
bool GameScene::InitNpcBuChuLable(){
	...
	const char * str;
	//npctwo标签
	str = ((String *)strings->objectForKey("buchu"))->getCString();
	CCLabelTTF* npcTwoBuChu = CCLabelTTF::create(str, "Helvetica-BoldOblique", 20);
	npcTwoBuChu->setPosition(m_npcTwoOut->getPoint());
	this->addChild(npcTwoBuChu, 1, NpcTwoBuChu);
	npcTwoBuChu->setVisible(false);
	//npcone标签
	CCLabelTTF* npcOneBuChu = CCLabelTTF::create(str, "Helvetica-BoldOblique", 20);
...
void GameScene::MovePk(Player* play, Poker* pk)
{
	...
	CCCallFuncND* func;
	float time = 0.05;
	play->getArrPk()->addObject(pk);//添加到实体牌库中
	//动画
...
void GameScene::SendPk(){
	...
	if(m_iSendPk<51 && m_isSend)
	{
		...
		if(m_iSendPk%3 == 0)//给玩家发牌
			...
			MovePk(m_npcTwo,pk);
		++m_iSendPk;
		m_isSend = false;//是否已发完牌，或者还未开始发牌
	...
	else if(m_iSendPk>53)
	{
		...
		m_iSendPk = 0;//恢复默认
		m_iState = 1;//是否已发完牌，进入叫地主状态
	}
}

//定时函数
void GameScene::update(float delta){
    //当前状态 0：发牌状态 1：叫地主状态 2：出牌状态 3：结果状态
	switch (m_iState)
	{
	case 0:
		SendPk();
...
bool GameScene::initAnNiu(){
	...
	const char * str;
	//一分
	str = ((String *)strings->objectForKey("yifen"))->getCString();
	MenuItemFont* yi = MenuItemFont::create(str,this,menu_selector(GameScene::menuCallbackYi));
	...
	//二分
	str = ((String *)strings->objectForKey("erfen"))->getCString();
	MenuItemFont* er = MenuItemFont::create(str,this,menu_selector(GameScene::menuCallbackEr));
	er->setAnchorPoint(ccp(0,0.5));
	er->setPosition(ccp(x+fontWidth.width+20,y));
	//三分
	str = ((String *)strings->objectForKey("sanfen"))->getCString();
	MenuItemFont* san = MenuItemFont::create(str,this,menu_selector(GameScene::menuCallbackSan));
	san->setAnchorPoint(ccp(0,0.5));
	san->setPosition(ccp(x+fontWidth.width*2+40,y));
	//不叫
	str = ((String *)strings->objectForKey("bujiao"))->getCString();
	MenuItemFont* buJiao = MenuItemFont::create(str,this,menu_selector(GameScene::menuCallbackBuJiao));
	...
	//不出
	str = ((String *)strings->objectForKey("buchu"))->getCString();
	MenuItemFont* buChu = MenuItemFont::create(str,this,menu_selector(GameScene::menuBuChu));
	buChu->setAnchorPoint(ccp(0,0.5));
	buChu->setPosition(ccp(x,y));
	//出牌
	str = ((String *)strings->objectForKey("chupai"))->getCString();
	MenuItemFont* chuPai = MenuItemFont::create(str,this,menu_selector(GameScene::menuChuPai));
	...
	//你胜了的按钮
	str = ((String *)strings->objectForKey("youwin"))->getCString();
	MenuItemFont* shengLi = MenuItemFont::create(str,this,menu_selector(GameScene::menuShengLi));
	...
	//你输了的按钮
	str = ((String *)strings->objectForKey("youlost"))->getCString();
	MenuItemFont* shu = MenuItemFont::create(str,this,menu_selector(GameScene::menuShengLi));
...
void GameScene::ShowFenShu(CCPoint pt,int num){
	...
	if (num == 3)
		str = ((String *)strings->objectForKey("sanfen"))->getCString();
	CCLabelTTF* showFenShu = CCLabelTTF::create(str, "Helvetica-BoldOblique", 20);
...
void GameScene::FenChaiNpcPai(Player* npc){
	...
	/* 1.首先分析出来牌的类型                                               */
	/************************************************************************/
	std::vector<PaiXing> vec;//存放所有牌型
	PaiXing xing;//牌型（包含牌型和包含的牌）
	...
	for(int i=0; i<arr->count();)
	{
		...
		if(xing.vec.size() == 4)
			xing.type = BOMB_CARD;//连续4张
		if(xing.vec.size() == 3)
			xing.type = THREE_CARD;//连续三张
		if(xing.vec.size() == 2)
			xing.type = DOUBLE_CARD;//连续两张
		if(xing.vec.size() == 1)
			xing.type = SINGLE_CARD;//单张
		//放入牌型库
		vec.push_back(xing);
	}
	/************************************************************************/
