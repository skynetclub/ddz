...
void Player::updatePkWeiZhi(){
	...
	int x,y;
	if(m_iPlayerClass == 0 || m_iPlayerClass == 3)//玩家，由之间位置求算，可视间距值加最后一张完全可视牌宽
	{
		x = size.width/2-((m_arrPk->count()-1)*pkJianJu+pkWidth)/2;
		y = m_point.y;
	}
	else if(m_iPlayerClass == 1 || m_iPlayerClass == 4 || m_iPlayerClass == 5)//电脑，即为初始位置
	{
		x = m_point.x;
		y = m_point.y;
	}
	else if(m_iPlayerClass == 2)//三张牌
	{
	...
	CCARRAY_FOREACH(m_arrPk,object){
		...
		{
			pk->showFront();
			pk->setPosition(ccp(x+num*pkJianJu+pkWidth/2,y));//除了间距，以牌为中心所以需要加上1/2牌宽
		}
